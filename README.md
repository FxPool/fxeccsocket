...
func main() {
	...
	config := &fxeccsocket.Config{
		PrivateKey:      privKey,
		UseEphemeralKey: false, // Use static key
	}

	// Listen for encrypted connections
	listener, err := fxeccsocket.Listen("tcp", ":8080", config)
	if err != nil {
		log.Fatal(err)
	}
	...
	for {
		...
		go handleConnection(conn.(*fxeccsocket.ECCConn))
	}
}

func handleConnection(conn *fxeccsocket.ECCConn) {
	defer conn.Close()
...
func main() {
	// Client can use ephemeral keys for forward secrecy
	config := &fxeccsocket.Config{
		UseEphemeralKey: true,
	}

	// Connect to encrypted server
	conn, err := fxeccsocket.Dial("tcp", "localhost:8080", config)
	if err != nil {
		log.Fatal(err)
	}
	defer conn.Close()
